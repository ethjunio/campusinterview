...
const Item: FC<any> = ({
  ...
  return (
    <Link {...{ href, as }}>
      <li
        className={cn}
        onClick={() => { hasMounted && isMobile && setSidebarStatus(!sidebarStatus); }}>
...
export const Sidebar: FC<{
  ...
  items: Array<[string, string, FC<{ className: string }>]>; // [title, href, Icon]
  ...
  return (
    <Sidebar
      ...
      items={[
        ...
        [t('nav.talentPool'), 'talent-pool', TalentPool],
        [
          t('nav.matching'),
          (width ?? 0) < 1024 ? 'matching/menu' : 'matching',
          Matching,
        ],
        [t('nav.chat'), (width ?? 0) < 1024 ? 'chatroom/menu' : 'chatroom', Chatroom],
