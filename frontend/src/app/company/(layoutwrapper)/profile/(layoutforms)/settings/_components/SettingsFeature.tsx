"use client";
import { useSaveProfileSettingsMutation } from "@/hooks/company/profile/useSaveProfileSettings";
import { useGetProfileSettingsQuery } from "@/hooks/company/profile/useGetProfileSettings";
...
export const SettingsFeature: FC = ({}) => {
  ...
  const createSettingsMutation = useSaveProfileSettingsMutation();

  const { data: settingsData, isLoading } = useGetProfileSettingsQuery();
  ...
  return (
    <>
      <h1>{t("companies.settings.title")}</h1>
          ...
          <div className="space-y-6 mt-8">
            <div className="flex space-x-2">
              <Switch
                ...
                width={40}
              />
              <div>{t("companies.settings.daily-notifications")}</div>
            </div>
            {/* <div className="flex space-x-2">
              <Switch
                onChange={(val) =>
            ...
            <div className="flex space-x-2">
              <Switch
                ...
                width={40}
              />
              <div>{t("companies.settings.keep-profile")}</div>
            </div>
          </div>
        </>
      )}
