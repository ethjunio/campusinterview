...
export const EconomyRoom: FC<Props> = ({
  ...
  const basePrice =
    previouslyBookedCount > 0 ? (room?.onlineCampusPrice || 0)  : room?.onlineCampusPrice || 0;
  const roomPrice =
    ...
    <BookingCard disabled={disabled}>
      ...
      <BookingCard.Body>
        <BookingCard.Body.Lead>
          {t('bookings.add-virtual-room-description')}
        </BookingCard.Body.Lead>
        <BookingCard.Body.List
          items={[
            t('bookings.add-virtual-room-list-1'),
            t('bookings.add-virtual-room-list-2'),
            t('bookings.add-virtual-room-list-3'),
            t('bookings.add-virtual-room-list-4'),
          ]}
        />
