...
const MatchesSection = () => {
  ...
  const emptySlots = Array(
    8 - (arrangedData?.data?.length ? arrangedData?.data?.length : 0)
  ).fill(null);

  return (
    <section className="w-full p-2">
      <div className="space-y-4">
        <h2 className="text-2xl font-bold !text-[24px] leading-[36px] text-[#101828] font-thin">Matches</h2>
  ...
  {/* LEFT BUTTON */}
  <button className="room-prev absolute left-1 top-1/2 -translate-y-1/2 z-20 bg-gray-100 text-[#0621B7] p-2 rounded-full">
    <ArrowLeft size={18} />
  </button>

  {/* RIGHT BUTTON */}
  <button className="room-next absolute right-1 top-1/2 -translate-y-1/2 z-20 bg-gray-100 text-[#0621B7] p-2 rounded-full">
    <ArrowRight size={18} />
  </button>
  ...
  >
    {matchesData?.data?.map((room: matchesRoomtype) => {
      ...
      const emptySlots = Array(9 - matchCount).fill(null);

      return (
        <SwiperSlide key={room.id}>

          <Link
                  href="/company/interviews"  className="grid grid-cols-3 gap-4 max-h-[218px] w-full">
            ...
            {filledSlots.map((_, index: number) => (
              <div
                ...
                h-[62px]
                rounded-2xl bg-PrimaryBlue text-white font-semibold"
              >
...
:<div className=" flex items-center justify-center mx-auto text-[#0621B7]">
  No rooms booked
  </div>}

        {/* RIGHT SIDE */}
        <Link  href="/company/profile/bookings/rooms" className="relative min-h-[260px] rounded-3xl border border-PrimaryBlue overflow-hidden flex items-center justify-center">
          {/* Background grid */}
          <div className="absolute inset-0 opacity-40 blur-sm p-4">
          ...
          >
            <Plus size={32} className="text-[#0621B7] mb-2" />
            <p className=" font-semibold text-lg text-[20px] leading-[26px] text-[#0621B7]">
