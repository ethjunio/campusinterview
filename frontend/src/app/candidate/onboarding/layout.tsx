...
export default function CandidateOnboardingLayout({
  ...
  const title = t('onboarding.information-title');
  const information = t('onboarding.information-text', { currentYear });

  const steps = stepRoutes.map((route) =>
    t(`onboarding.step-${route}-title`)
  );
      ...
      <section className="px-8 lg:px-16 mb-20 lg:mb-32 flex flex-grow">
        <div className="mt-4 lg:mt-16 w-full lg:w-auto">
          ...
          {/* <div className="hidden lg:block">
            <Steps steps={steps} selected={currentStep} type="candidate" />
          </div> */}

          {/* Mobile Circular Progressbar */}
          {/* <div className="flex justify-center w-full">
            <CircularProgressbar
              styles={buildStyles({})}
              className="lg:hidden w-32 mt-6 mb-10"
              value={percentageStepsCompleted}
              text={`${Math.round(percentageStepsCompleted)}%`}
            />
          </div> */}
