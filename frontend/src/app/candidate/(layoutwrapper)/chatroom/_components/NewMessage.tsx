...
const NewMessage: React.FC<NewMessageProps> = ({
  ...
  const { sendMessage } = useWebSocket(userId,companyId);
  const { sendMessageAwait } = useWebSocket(userId,companyId);
  ...
  async function uploadMessageDocs(reply: any, filesMap: any): Promise<any> {
    ...
    reply.files?.forEach((item: any, idx: any) => {
      ...
      // provide the filename too—some servers need it
  ...
  return (
    <div className="w-full flex py-2 lg:py-5 items-center">
      ...
      <div className="flex-1 relative mr-3">
        ...
        <div className="border-2 border-gray-300 rounded-md py-0.5 px-3 md:pr-12 relative bg-white">
          {/* TEXTAREA */}
          <textarea
            ref={inputEl}
      ...
      <div className="hidden sm:block">
        ...
        >
          {t("candidate.chatroom.send-message")}
        </Button>
      </div>
