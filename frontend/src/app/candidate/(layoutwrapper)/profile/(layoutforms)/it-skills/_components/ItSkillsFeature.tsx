...
const ItSkillsFeature = () => {
  ...
  const { data: experienceData } = useGetCandidateExperienceQuery();

  const experienceOptions = useMemo(
    () =>
      (experienceData?.data || []).map((e: any) => ({
        value: e.id,
        label: e.employer ? `${e.title} — ${e.employer}` : e.title,
      })),
    [experienceData]
  );
      ...
      itSkills:
        data.data && data.data.length
          ? data.data.map((s: any) => ({
              ...
              experienceIds: (s.linkedExperiences || []).map((e: any) => ({
                value: e.id,
                label: e.title,
              })),
            }))
