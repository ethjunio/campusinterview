...
import UploadCompanyLogoForm from "./UploadCompanyLogoForm";
...
export default function LiveLogosCarousel() {
  const t = useTranslations();
  const queryClient = useQueryClient();
  const logoFormRef = useRef<{ reset: () => void }>(null);

  const { data, isPending } = useGetCompanyLogoQuery("live");
  const deleteCompanyLogoMutation = useDeleteCompanyLogoMutation({
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["companyLogos", "live"] });
  ...
  return (
    <>
      <h2>{t("admin.company-logos-edit.live-logos.title")}</h2>
      ...
      <hr className="mt-4 mb-4" />
      <h3>{t("admin.company-logos-edit.live-logos.list-title")}</h3>
      ...
      <hr className="mt-4 mb-4" />
      <h3>{t("admin.company-logos-edit.live-logos.carousel-title")}</h3>
