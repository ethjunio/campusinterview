...
// import { getFaqs, saveFaqs, type FAQ } from "@/lib/faqStore";

import { ArrowLeft, DeleteIcon, Pencil, Plus, Trash2 } from "lucide-react";
...
import { useGetFaqsCompanyBoxQuery } from "@/hooks/admin/useGetFaqsCompanyBoxQuery";
...
const FaqCompany = () => {
  ...
  const { data, isPending } = useGetFaqsCompanyBoxQuery();
  ...
  const deleteFaqMutation = useDeleteFaqsMutation({
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["getFaqCompanyBox"] });
    },
    onError: (err: any) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-6 w-1/2">
      <h1 className="pb-16">{t("admin.faq-section-company")}</h1>
