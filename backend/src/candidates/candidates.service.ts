...
export class CandidatesService {
  ...
  private async getCandidate(email: string): Promise<CandidateDocument> {
    ...
    if (!candidate)
      candidate = await this.candidateModel.create({ userId: user._id });
    return candidate;
  }
  ...
  public async getPersonalData(email: string): Promise<PersonalDataDto | null> {
    ...
    const personal = plainToInstance(PersonalDataDto, candidate.personal, {
    ...
    personal.residencePermitId = getIndexByPermitValue(
  ...
  public async setPersonalData(email: string, personal: PersonalDataDto) {
    ...
    candidate.personal.nationality = getCountryCodeByIndex(
      personal.nationalityId,
    )?.alpha2;
    candidate.personal.residencePermit = getPermitByIndex(
  ...
  public async getEducation(
    email: string,
  ): Promise<EducationDetailsDto | null> {
    const candidate = await this.getCandidate(email);
    if (!candidate.education) return null;
    return plainToInstance(EducationDetailsDto, candidate.education, {
      excludeExtraneousValues: true,
    });
  }
  ...
  public async getExperiences(email: string): Promise<ExperiencePositionDto[]> {
    ...
    return candidate.experiences.map((p) => {
      const dto = plainToInstance(ExperiencePositionDto, p, {
        excludeExtraneousValues: true,
      });
  ...
  public async getExtracurriculars(
    email: string,
  ): Promise<ExtracurricularPositionDto[]> {
    const candidate = await this.getCandidate(email);
    if (!candidate.extracurriculars) candidate.extracurriculars = [];
    return candidate.extracurriculars.map((p) => {
      const dto = plainToInstance(ExtracurricularPositionDto, p, {
        excludeExtraneousValues: true,
      });
      return dto;
    });
  ...
  public async setExtracurriculars(
    email: string,
    positions: ExtracurricularPositionDto[],
  ) {
    const candidate = await this.getCandidate(email);
    candidate.extracurriculars = [];
    for (const position of positions)
      candidate.extracurriculars.push({ ...position });
    await candidate.save();
    await this.updateOnboardingState(email);
  }
  ...
  public async getSkills(email: string): Promise<SkillPositionDto[]> {
    ...
    return candidate.skills.map((p) => {
      const dto = plainToInstance(SkillPositionDto, p, {
        excludeExtraneousValues: true,
      });
      dto.itSkill = { id: p.itSkillId, name: skillOptions[p.itSkillId]?.label };
      dto.skillLevel = {
        id: p.skillLevelId,
        name: skillLevelsOptions[p.skillLevelId]?.label,
      };
      return dto;
    });
  }
  ...
  public async getLanguages(email: string): Promise<LanguagePositionDto[]> {
    ...
    return candidate.languages.map((p) => {
      const dto = plainToInstance(LanguagePositionDto, p, {
        excludeExtraneousValues: true,
      });
      dto.language = { code: p.languageCode, name: p.languageCode };
      dto.languageLevel = {
        id: p.languageLevelId,
        name: skillLevelsOptions[p.languageLevelId]?.label,
      };
      return dto;
    });
  }
  ...
  public async setJobRequirements(
    email: string,
    _requirements: JobRequirementsDto,
  ) {
    const candidate = await this.getCandidate(email);
