...
export const countryCodes: CountryCode[] = [
  ...
  { name: "Netherlands, Kingdom of the", alpha2: "NL" },
...
export function getIndexByAlpha2(alpha2: string | null | undefined): number | undefined {
  if (alpha2 === null || alpha2 === undefined) return undefined;
  const index = countryCodes.findIndex(option => option.alpha2 === alpha2);
  return index !== -1 ? index : undefined;
}

export function getCountryCodeByIndex(index: number | string | null | undefined): CountryCode | undefined {
  if (index === null || index === undefined) return undefined;
  const parsedIndex = typeof index === 'string' ? parseInt(index, 10) : index;
  if (isNaN(parsedIndex) || parsedIndex < 0 || parsedIndex >= countryCodes.length) {
    return undefined;
  }
  return countryCodes[parsedIndex];
}
