...
export const educationLevelOptions: EducationLevelOption[] = [
  ...
  { value: 'bachelor', label: "Bachelor's Degree (University / ETH / FH)" },
  {
    ...
    label: 'Advanced Federal Diploma / College of Higher Education (HF / FA)',
  },
  {
    value: 'vocational_training',
    label: 'Federal Diploma of Vocational Education and Training (EFZ)',
  },
  {
    value: 'high_school',
    label: 'High School / Matura / Gymnasialer Abschluss',
  },
  { value: 'lower_secondary', label: 'Secondary School / Sekundarstufe I' },
  { value: 'other', label: 'Other / No Formal Degree' },
];

export function getEducationByIndex(
  index: number | string | null,
): EducationLevelOption | null {
  if (index === null || index === undefined) return null;
  const parsedIndex = typeof index === 'string' ? parseInt(index, 10) : index;
  if (
    isNaN(parsedIndex) ||
    parsedIndex < 0 ||
    parsedIndex >= educationLevelOptions.length
  ) {
    return null;
  }
...
export function getEducationIndexByValue(value: string | null): number | null {
  ...
  const index = educationLevelOptions.findIndex(
